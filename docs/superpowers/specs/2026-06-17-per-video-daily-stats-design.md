...
`channel` onto each emitted item **only as the routing key** (used to set the
tab name; not written into the sheet).
...
enabled; map the 16 data columns by header. The `channel` field is the tab
selector only — the **tab name alone identifies the channel**, so it is not
itself a column.

**Live-only workflow.** `ENbQm9ctfNRcnOuT` is **not** regenerated by
`n8n/generate.py`; it lives only in n8n (like the Error-Alert workflow). The new
branch is applied to the **live** workflow via the n8n API, following the
established surgical-update pattern (`scripts/patch_telegram_parsemode.py` style)
and the known n8n API gotchas: drop newer `settings` keys on PUT, `errorWorkflow`
is silently dropped (set in UI), and the workflow must be re-published after the
update.
...
- **Tab name alone identifies the channel** — `channel` is not written as a
  column; it is used only to select the destination tab.
- **The Digest workflow is live-only** — patched via the n8n API, not regenerated
  by `n8n/generate.py` (see §5).
