...
- **Live sentence-level captions** (TikTok-style, advancing with the voice) — acoustically aligned to the narration when `ALIGN_BACKEND=forced` (`torchaudio` MMS_FA forced alignment, ~1.18 GB model on first use); defaults to proportional word-count timing
...
│   │   ├── alignment.py          # forced caption alignment (torchaudio MMS_FA)
...
| Image gen | **Z-Image-Turbo** (Tongyi-MAI) | Apache-2.0-ish | **Production: Modal serverless L4 GPU** (`image_backend=modal`, ~15 s/image, ~$0.06/run) → free hosted ZeroGPU Space → local mflux (~5 min/image, ~28 GB unified memory), automatic per-image fallback. Configure via `image_backend` in `api/config.py`. See [`modal_app/README.md`](modal_app/README.md) and AGENTS.md §2.13/§2.17. |
| TTS | **Piper** — 3-voice random pool: john, bryce, joe | Public domain / CC0 | YouTube-monetization safe; voice shuffled per video for variety. Optional higher-quality **Kokoro** backend via `voice_backend` (Docker container, Piper fallback) — see AGENTS.md §2.15 |
